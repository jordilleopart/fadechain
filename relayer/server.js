...
/**
 * GET /
 * Root route - returns basic info about the relayer
 */
app.get("/", (req, res) => {
  res.json({
    service: "ZK Time-Decay Voting Relayer",
    version: "1.0.0",
    status: "running",
    endpoints: [
      "GET /api/status - Get contract status",
      "GET /api/health - Health check",
      "GET /api/results - Get voting results",
      "POST /api/register - Register voter commitment",
      "POST /api/relay-vote - Submit vote via relayer",
    ],
...
app.get("/api/status", async (req, res) => {
  try {
    console.log(`📊 Status request - Contract: ${VOTING_CONTRACT_ADDRESS}`);
    ...
    console.log(`   Phase: ${regOpen ? 'registration' : 'voting/ended'}, Votes: ${totalVotes}`);
    ...
    console.error("   Contract address:", VOTING_CONTRACT_ADDRESS);
    ...
    res.status(500).json({ 
      ...
      contractAddress: VOTING_CONTRACT_ADDRESS,
      rpcUrl: RPC_URL,
      hint: "Check that VOTING_CONTRACT_ADDRESS is correct and deployed on the network"
    });
  }
});
